...
import { Lock, Zap, Database, Target, Activity, CheckCircle, AlertCircle, Hash, Shield, Globe } from 'lucide-react';
...
const HashingEngineDemo: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'console' | 'integration' | 'performance'>('console');
  const [integrationTests, setIntegrationTests] = useState<IntegrationTest[]>([
    {
      id: 'threat-intel',
      name: 'Threat Intelligence Hashing',
      description: 'Hash IOCs and threat indicators for secure storage and comparison',
      status: 'pending'
    },
    {
      id: 'document-manager',
      name: 'Document Manager (USIM) Integration',
      description: 'Hash documents for integrity verification and blockchain coordination',
      status: 'pending'
    },
    {
      id: 'legion-tasks',
      name: 'Legion Task Script Hashing',
      description: 'Hash adversary/counter-adversary scripts across 4 operational worlds',
      status: 'pending'
    },
    {
  ...
  const testThreatIntelHashing = async (): Promise<string> => {
    ...
    return `Hashed ${result.completedCount}/${indicators.length} threat indicators. Avg processing: ${(result.totalProcessingTime / indicators.length).toFixed(2)}ms per indicator.`;
  };
...
Timestamp: ${new Date().toISOString()}`
    ...
    return `Hashed ${result.completedCount} documents. Average compression: ${avgCompression.toFixed(1)}%. Ready for USIM blockchain coordination.`;
  };
  ...
  const testDatabaseIntegrityHashing = async (): Promise<string> => {
    const dbRecords = [
      JSON.stringify({ table: 'comprehensive_crate_interviews', record_count: 127, last_modified: Date.now() }),
      JSON.stringify({ table: 'node_interviews', record_count: 165, last_modified: Date.now() }),
      JSON.stringify({ table: 'kali_tools', record_count: 89, last_modified: Date.now() }),
      JSON.stringify({ table: 'threat_intelligence', record_count: 234, last_modified: Date.now() })
    ];
      ...
      if (response.ok) {
        ...
        if (status.performance) {
          setPerformanceMetrics({
            ...
            compressionRatio: status.performance.totalCompressions / Math.max(status.performance.totalRequests, 1) * 100,
    ...
    <div className="h-screen bg-gray-900">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">Hashing Engine Integration</h1>
                <p className="text-gray-400">
                  Containerized Rust Implementation • CTAS-7 Ops Platform Integration • Port 18005
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <div className="text-right">
                <div className="text-2xl font-bold text-green-400">{performanceMetrics.throughput}</div>
                <div className="text-xs text-gray-400">req/sec</div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-blue-400">{performanceMetrics.avgProcessingTime}ms</div>
                <div className="text-xs text-gray-400">avg time</div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-purple-400">{performanceMetrics.compressionRatio.toFixed(1)}%</div>
      ...
      <div className="h-full">
          ...
          <div className="p-6">
            <div className="max-w-4xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-semibold text-white">CTAS System Integration Tests</h2>
                <button
                  onClick={runAllTests}
                  ...
                  <div key={test.id} className="bg-gray-800 rounded-lg p-4">
                    ...
                    {test.result && (
                      <div className={`text-sm p-2 rounded ${
                        test.status === 'success' ? 'bg-green-900/20 text-green-300' : 'bg-red-900/20 text-red-300'
                      }`}>
                        {test.result}
                        {test.duration && (
                          <span className="ml-2 text-gray-400">({test.duration}ms)</span>
                        )}
                      </div>
                    )}
                  </div>
        ...
        {activeTab === 'performance' && (
          <div className="p-6">
            <div className="max-w-4xl mx-auto">
              <h2 className="text-xl font-semibold text-white mb-6">Performance Monitoring</h2>

              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-gray-800 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-blue-400">{performanceMetrics.totalRequests}</div>
                  <div className="text-sm text-gray-400">Total Requests</div>
                </div>
                <div className="bg-gray-800 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-green-400">{performanceMetrics.avgProcessingTime}ms</div>
                  <div className="text-sm text-gray-400">Avg Processing Time</div>
                </div>
                <div className="bg-gray-800 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-purple-400">{performanceMetrics.compressionRatio.toFixed(1)}%</div>
                  <div className="text-sm text-gray-400">Compression Efficiency</div>
                </div>
                <div className="bg-gray-800 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-yellow-400">{performanceMetrics.throughput}</div>
                  <div className="text-sm text-gray-400">Requests/Second</div>
                </div>
              </div>

              <div className="bg-gray-800 rounded-lg p-6">
                <h3 className="text-lg font-semibold text-white mb-4">Integration Capabilities</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center space-x-3">
                    <Shield className="w-5 h-5 text-red-400" />
                    <span className="text-gray-300">Threat Intelligence IOC Hashing</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Database className="w-5 h-5 text-blue-400" />
                    <span className="text-gray-300">Document Manager (USIM) Integration</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Target className="w-5 h-5 text-purple-400" />
                    <span className="text-gray-300">Legion Task Script Verification</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Globe className="w-5 h-5 text-green-400" />
                    <span className="text-gray-300">4-World Operational Context</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Hash className="w-5 h-5 text-yellow-400" />
                    <span className="text-gray-300">95%+ Compression Ratios</span>
                  </div>
                  <div className="flex items-center space-x-3">
                    <Zap className="w-5 h-5 text-orange-400" />
                    <span className="text-gray-300">High-Performance Rust Engine</span>
                  </div>
                </div>
              </div>
            </div>
