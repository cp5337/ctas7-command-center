...
export class OrbitalAnimationManager {
  ...
  private updateSatellitePositions(): void {
    ...
    this.satellites.forEach((sat) => {
      ...
      while (geodetic.longitude > 180) geodetic.longitude -= 360;
      while (geodetic.longitude < -180) geodetic.longitude += 360;

      // Clamp latitude to -90 to 90 range
      if (Math.abs(geodetic.latitude) > 90) {
        console.warn(`Out of bounds latitude for satellite ${sat.id}:`, geodetic.latitude);
        return;
      }
  ...
  private createOrUpdateLaserLink(
    ...
    // Normalize longitude to -180 to 180 range
    while (satLon > 180) satLon -= 360;
    while (satLon < -180) satLon += 360;

    // Check latitude bounds
    if (Math.abs(satLat) > 90) {
      console.warn(`Out of bounds latitude for laser link:`, { satLat });
      return;
    }
